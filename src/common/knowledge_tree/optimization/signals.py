...
class OptimizationSignal:
    ...
    signal_type: str  # "total_failure" | "rag_false_positive" | "content_insufficient"
    node_id: str | None
...
SIGNAL_PRIORITY: dict[str, int] = {
    "total_failure": 1,
    "rag_false_positive": 2,
    "content_insufficient": 3,
}
...
def detect_signals(
    ...
    for log in logs:
        # 整体失败（RAG 无结果）
        if not log.rag_results:
            total_failure_count += 1
            total_failure_queries.append(log.query_text)

        # RAG 假阳性（Agent 明确标注不满意）
        if log.rag_results and log.agent_satisfaction is False:
