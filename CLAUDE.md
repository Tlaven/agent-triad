...
`execute_plan` 工具接受 LLM 传入的**结构化参数**：
- **Mode 2（Tool-use ReAct）**：只传 `task_description`，极简参数
- **Mode 3（Plan → Execute）**：优先只传 `plan_id`（极大节省 token），兜底时传 `plan_json`

`generate_plan` 接受 LLM 传入的结构化参数：
- `task_core`：Supervisor 提炼后的精简 intent
- `plan_id`：当前 Plan 的编号（指向 session.plan_json，即 previous_plan）

**原因**：
- 通过 `plan_id` 传递实现极低 token 消耗（Mode 3）
- `task_description` 极简传参保证 Supervisor → Executor 高效通信（Mode 2）
...
Supervisor 收到 Executor 结果
  ...
  └── status=failed
        ├── replan_count < MAX_REPLAN → 调 generate_plan（传 plan_id）→ 再 execute_plan（传 plan_id）
