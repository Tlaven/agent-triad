...
> **2026-07-02 后续验证（N2 反向排除）**：`tests/unit_tests/supervisor_agent/test_wait_for_executor_result.py` 4/4 pass——隔离环境下 `_wait_for_executor_result` 的 wait loop timeout（`tools.py:678-692`）**真的会 fire**，probe unreachable/not_found 分支**真的立即返回**。所以 hang **不在本函数内部**，必然在上层（dev server 热重载 N1 / LangGraph API 层 worker 状态滞后）。修复优先级：**N1 缓解（探测改 `--no-reload`）**，supervisor 代码层不再追查。
