...
[V4] 加固：LLM 调用超时保护    ✅
     ...
     - 摄入质量门槛（决策 29）：宁缺毋滥策略 + 三级垃圾检测 + 经验密度门控 + RAG 矛盾截断 + 知识树清理（73→25）
     - LLM 调用超时保护（决策 30）：三 Agent 独立超时配置 + API 预热 + 调用耗时日志
     - E2E 长对话验证：17 轮次 × 10 维度全通过（Mode 1/2/3、上下文连贯、歧义处理、错误恢复、KT 摄入/检索、安全拒绝、数学推理、自我认知）
     ...
     - 架构决策：决策 18-30（architecture-decisions.md）
```
