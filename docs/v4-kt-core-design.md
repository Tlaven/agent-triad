...
**元知识自举的现实约束**（测试 F7-F9 验证，F7 已通过双通道注入突破）：

1. **系统提示词是前提**：RAG 注入的元知识被 LLM 当"信息"而非"指令"。只有系统提示词明确要求"遵循注入的行为规则"时，元知识才能有效影响决策。**已通过双通道注入解决**：持久元规则走系统提示通道（指令通道），普通知识走用户消息通道（信息通道）。
2. ~~**只能优化，不能触发**~~：~~元知识可以优化已有行为，但不能触发新行为。~~ **已突破（2026-05-21 E2E 5/5 通过）**：通过 `knowledge_tree_add_meta_rule` 创建持久元规则，注入到系统提示中，成功触发新行为（用户分享信息 → 主动 ingest）。元规则绕过相似度阈值，每次请求无条件注入。
3. **认知集成的四层结构**：
   - 系统提示词（告知 Supervisor 有 KT、何时使用）→ 不可替代的基础
   - Auto-inject（被动记忆注入）→ 需要明确标示来源
   - 元知识 RAG（运行时策略优化）→ 需要前两层支撑才能生效
   - **持久元规则（新增第四层）**→ 系统提示通道强制注入，可触发新行为、改变操作模式
